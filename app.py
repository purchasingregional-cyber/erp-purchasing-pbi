...
# Versi: 2.5 (Stable Original ERP Version + Adaptive Mode)
# ==============================================================================
...
# Pengaturan Style CSS (ADAPTIF LIGHT & DARK MODE)
st.markdown("""
    <style>
    ...
    /* Kotak Metric Adaptif (Transparan menyesuaikan tema) */
    .stMetric {
    ...
    /* Desain Button Tetap Hijau Khas Panca Budi */
    .stButton>button {
        width: 100%;
        border-radius: 8px;
        height: 3em;
...
if selected_menu == "Pembersihan PO":
    ...
    input_unit = st.selectbox(
        "🏢 Pilih Unit Kerja (Default):", 
        ["PBI CPR", "PBI PML", "PBI MAUK", "PP CUP", "PIH", "RA", "PGP", "OFFICE PUSAT"],
        help="Tentukan unit asal data jika sistem tidak mendeteksi dari nama file."
    )
    ...
    if uploaded_file:
        try:
            ...
            st.info("Memproses Laporan ERP (Mencari pola No Bukti / Transaksi)...")
            ...
            if not df_final_clean.empty:
                ...
                st.warning("⚠️ Tidak ada data yang berhasil diekstrak. Pastikan file Excel sesuai dengan format ERP Standar (memiliki keterangan INCLUDE/EXCLUDE).")
...
st.markdown(
    ...
    "ERP Purchasing System v2.5 | Proprietary of PT Panca Budi Idaman Tbk | Created with for Raihan Subakti"
    "</p>", 
    unsafe_allow_html=True
)