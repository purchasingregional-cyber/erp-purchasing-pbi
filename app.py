...
# Versi: 2.7 (Manual Enter Button + Robust Data Extraction)
# ==============================================================================
...
def parse_numeric(value):
    """Fungsi ekstraksi angka yang JAUH LEBIH KUAT agar tidak menyerah pada nilai 0"""
    try:
        if pd.isna(value) or str(value).strip() == "": return None
        s = str(value).strip()
        
        # Cegah tanggal terbaca sebagai angka
        if re.search(r'\d{2}/\d{2}/\d{4}|\d{4}-\d{2}-\d{2}', s): return None
        # Pastikan isinya hanya angka, minus, titik, dan koma (Bukan Teks murni)
...
if selected_menu == "Pembersihan PO":
    st.markdown("## ✨ Pembersihan Data PO")
    ...
    # MENGGUNAKAN FORM AGAR ADA TOMBOL ENTER
    ...
    # JIKA TOMBOL DITEKAN DAN ADA FILE
                ...
                for idx, row in raw_excel.iterrows():
                    ...
                    if temp_po != "-":
                        ...
                        if len(val_list) >= 4:
                            ...
                            # AMBIL ANGKA DENGAN FUNGSI ANTI-ERROR
                            numeric_finds = []
                            for v in reversed(val_list):
                                n = parse_numeric(v)
                                if n is not None: numeric_finds.insert(0, n)
    ...
    # JIKA DATA MENTAH SUDAH BERHASIL DIEKSTRAK
...
st.markdown(
    ...
    "ERP Purchasing System v2.7 | Proprietary of PT Panca Budi Idaman Tbk | Created with for Raihan Subakti"
    "</p>", 
    unsafe_allow_html=True
)