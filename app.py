...
# Versi: 13.4 (ULTIMATE FULL VERSION - Syntax & Start_Idx Bug Fix)
...
if menu == "Pembersihan PO":
    ...
    if btn_proses and file_raw:
        try:
            ...
            if "Plant RA" in pilihan_format: st.info("🤖 Mesin Smart-Detect RA sedang memindai format (Lama/Baru) pada seluruh sheet...")
            elif "Plant PGP" in pilihan_format: st.info("🤖 Mesin Smart-Detect PGP sedang memindai format (Lama/Baru) pada seluruh sheet...")
            elif "Plant Tangerang" in pilihan_format: st.info("🤖 Mesin Smart-Detect Tangerang sedang memindai format (Lama/Baru) pada seluruh sheet...")
            elif "Plant Pemalang" in pilihan_format: st.info("🤖 Mesin Smart-Detect Pemalang sedang memindai format (Lama/Baru) pada seluruh sheet...")
            elif "Plant Solo" in pilihan_format: st.info("🤖 Mesin Smart-Detect Solo sedang memindai format pada seluruh sheet...")
            elif "PIHC" in pilihan_format: st.info("🤖 Mata Pisau Khusus PIHC menjahit kolom beda baris di seluruh sheet...")
            elif "ERP Pusat" in pilihan_format: st.info("🤖 Mesin ERP Pusat membaca seluruh sheet...")
                    ...
                    for idx, row in df_input.head(30).iterrows():
                        for i, c in enumerate(row.values):
                            ...
                            if ('JENIS BARANG' in x_clean or 'NAMA BARANG' in x_clean) and col_nama == -1: 
                                ...
                                col_qty = i
                            elif ('SATUAN' in x_clean) and col_sat == -1:
                                col_sat = i
            ...
            if not df_to_export.empty:
                ...
                for _, r in df_to_export.iterrows():
                    ...
                    export_data.append({
                        ...
                        'QTY': r['QTY'], 'UOM': r['SATUAN'], 'HARGA SATUAN': r['HARGA'], 
        ...
        if st.button("🔄 Generate Preview SKU", type="primary"):
            with st.spinner("Membuat draft SKU..."):
                try:
                    ...
                    # --- V13.4 FIX: DYNAMIC LEVEL DETECTION ---
...
st.markdown(
    ...
    f"ERP Purchasing System v13.4 | Proprietary of PT Panca Budi Idaman Tbk | Created with ❤️ for Raihan Subakti<br>"
